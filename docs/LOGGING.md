...
审计日志与任务执行日志是两个独立的日志体系。任务日志记录的是任务执行过程中的输出信息，审计日志记录的是用户在系统中的操作行为。

### 架构概览

```
┌─────────────────────────────────────────────────────────────────────────┐
│                        用户请求 (POST/PUT/DELETE)                       │
└─────────────────────────────────────────────────────────────────────────┘
                                   │
                                   ▼
┌─────────────────────────────────────────────────────────────────────────┐
│                      JWT 认证中间件                                      │
│  提取 user_id, username, role, domain_id                                │
└─────────────────────────────────────────────────────────────────────────┘
                                   │
                                   ▼
┌─────────────────────────────────────────────────────────────────────────┐
│                      审计日志中间件                                      │
│  1. 仅拦截 POST/PUT/DELETE 请求                                         │
│  2. c.Next() 执行后续处理                                                │
...
┌─────────────────────────────────────────────────────────────────────────┐
│                      审计日志服务                                        │
│  AuditLogService.Create() 写入 bdopsflow_audit_logs 表                   │
│  AuditLogService.List() 多条件筛选和分页                                  │
│  AuditLogService.CleanExpired() 清理过期日志                              │
└─────────────────────────────────────────────────────────────────────────┘
                                   │
                                   ▼
┌─────────────────────────────────────────────────────────────────────────┐
│                      定时清理协程                                        │
│  每24小时自动清理过期审计日志                                              │
│  保留天数由 audit_log.retention_days 系统配置控制（默认90天）              │
└─────────────────────────────────────────────────────────────────────────┘
```
...
| 维度 | 任务日志 (bdopsflow_task_logs) | 审计日志 (bdopsflow_audit_logs) |
|------|------|------|
| 用途 | 记录任务执行过程输出 | 记录用户操作行为 |
| 触发方式 | 执行器上报 | 中间件自动捕获 |
| 数据来源 | Shell stdout/stderr, HTTP 响应 | HTTP 请求上下文 |
| 查看权限 | 已登录用户 | 仅系统管理员 |
| 传输方式 | gRPC → 数据库 → SSE | 中间件 → 异步写入 |
| 保留策略 | 建议保留7-30天 | 默认保留90天 |
| 日志级别 | info/warn/error/stdout/stderr | success/failure |

### 路由解析规则

审计中间件通过三级规则解析操作的 resource 和 action：

1. **精确匹配** (`routeAuditRules`): 完整路径匹配，如 `/api/auth/login` → auth/login
2. **前缀匹配** (`routePrefixRules`): 路径前缀匹配，如 `/api/tasks/` → task
3. **关键词推断**: 路径关键词推断 action，如 `/trigger` → trigger, `/reset-password` → reset_password

Handler 可通过 `c.Set()` 覆盖默认解析结果：
- `audit_action`: 覆盖操作类型
- `audit_resource`: 覆盖资源类型
- `audit_resource_id`: 设置资源ID
- `audit_resource_name`: 设置资源名称
- `audit_detail`: 设置操作详情

### 审计日志查询

审计日志管理页面位于 `/admin/audit-logs`，仅系统管理员可访问。
...
- **自动清理**: 调度中心启动时创建定时协程，每24小时清理一次
- **手动清理**: 系统管理员可通过 API 手动触发清理
- **保留天数**: 通过 `audit_log.retention_days` 系统配置控制，默认90天
- **清理逻辑**: `DELETE FROM bdopsflow_audit_logs WHERE created_at < datetime('now', '-N days')`
