...
watch(() => props.executionId, async (newId) => {
  if (newId) {
    logs.value = []
    realtimeOutput.value = props.output || ''
    realtimeError.value = props.error || ''
...
// 监听 props 的变化，同步到实时变量
watch(() => props.output, (val) => {
  if (val && !realtimeOutput.value) {
    realtimeOutput.value = val
  }
})

watch(() => props.error, (val) => {
  if (val && !realtimeError.value) {
    realtimeError.value = val
  }
})
