...
func TestPermissionModel(t *testing.T) {
	...
	t.Run("GetAllPermissionGroups 返回所有权限分组", func(t *testing.T) {
		groups := GetAllPermissionGroups()
		...
		expectedResources := []string{"user", "role", "permission", "domain", "executor", "task", "log", "workflow"}
	...
	t.Run("权限分组包含正确的操作", func(t *testing.T) {
		groups := GetAllPermissionGroups()

		for _, group := range groups {
			if group.Resource == "task" {
				// 任务权限应该包含 create, read, update, delete, trigger, manage
				actions := make(map[string]bool)
				for _, perm := range group.Permissions {
					actions[perm.Action] = true
				}

				expectedActions := []string{"create", "read", "update", "delete", "trigger", "manage"}
				for _, action := range expectedActions {
					if !actions[action] {
						t.Errorf("任务权限应包含操作 '%s'", action)
					}
				}
			}
		}
