...
func (d *MySQLDriver) Connect(ctx context.Context, config DatasourceConfig) error {
	d.config = config
	port := config.Port
	if port == 0 {
		port = 3306
	}
	slog.Debug("mysql connecting", "host", config.Host, "port", port, "database", config.Database)
	dsn := d.buildDSN()
	db, err := sql.Open("mysql", dsn)
	if err != nil {
		slog.Error("mysql connection failed", "host", config.Host, "port", port, "error", err)
	...
	d.db = db
	slog.Info("mysql connected", "host", config.Host, "port", port, "database", config.Database)
	return nil
}
...
func (d *MySQLDriver) GetTables(ctx context.Context, database string) ([]TableInfo, error) {
	if database == "" {
...
func (d *MySQLDriver) GetColumns(ctx context.Context, database, table string) ([]ColumnInfo, error) {
	if database == "" {
		database = d.config.Database
	}
	query := fmt.Sprintf(
...
func (d *MySQLDriver) SupportsCancel() bool {
	return true
}
...
func (d *MySQLDriver) TryQueryWithDB(ctx context.Context, query string, database string) (*QueryResult, error) {
...
func (d *MySQLDriver) UseDatabase(ctx context.Context, database string) error {
	...
	if err != nil {
		return fmt.Errorf("mysql use database error: %w", err)
	}
	slog.Debug("mysql switched database", "database", database)
	return nil
}

func (d *MySQLDriver) buildDSN() string {
	host := d.config.Host
	port := d.config.Port
	if port == 0 {
		port = 3306
