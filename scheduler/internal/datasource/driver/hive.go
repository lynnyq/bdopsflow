...
func (d *HiveDriver) QueryWithDB(ctx context.Context, query string, database string) (*QueryResult, error) {
	...
	case <-ctx.Done():
		...
		// 超时后仍继续，由 discard 机制兜底
		select {
		case <-resultCh:
		case <-time.After(3 * time.Second):
			slog.Warn("hive query goroutine did not exit in time after cancel, discarding connection", "sql_preview", truncateSQL(normalizedQuery, 200))
		}
